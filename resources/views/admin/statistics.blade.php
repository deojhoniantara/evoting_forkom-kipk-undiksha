...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
    <!-- Votes by Candidate -->
    <div class="bg-white rounded-2xl shadow-2xl p-8 hover:shadow-primary/30 transition-all duration-300">
    ...
    <!-- Voting Progress -->
    <div class="bg-white rounded-2xl shadow-2xl p-8 hover:shadow-accent/30 transition-all duration-300">
        <h2 class="text-lg font-bold text-primary mb-4 flex items-center gap-2">
        ...
        <div class="h-80">
            <canvas id="votingProgressChart"></canvas>
        </div>
        <div class="mt-4 flex justify-center gap-4">
            <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-primary/10 text-primary">
                Voted: <span class="ml-1 font-bold">{{ $totalVotes }}</span>
            </span>
            <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-gray-100 text-gray-700">
                Not Voted: <span class="ml-1 font-bold">{{ $totalVoters - $totalVotes }}</span>
            </span>
        </div>
    </div>
</div>
...
new Chart(votesByCandidateCtx, {
    ...
    options: {
        ...
        scales: {
            x: {
                grid: { display: false },
                ticks: { color: '#4576D3', font: { weight: 'bold' } }
            },
            y: {
                beginAtZero: true,
                grid: { color: '#E5E7EB' },
                ticks: { stepSize: 1, color: '#4576D3', font: { weight: 'bold' } }
            }
        }
    }
});
